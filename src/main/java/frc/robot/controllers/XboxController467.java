...
public final class XboxController467 extends XboxController {
   ...
   *
   * @param speed the drive speed without taking triggers into account.
   * @return the drive speed, taking the turbo and slow triggers into account.
   */
  ...
  /**
   * Returns the drive speed, with fast speed being taken into account.
   *
   * @param speed the drive speed without taking triggers into account.
   * @return the drive speed, with fast speed being taken into account.
   */
  public double turboFastSpeed(final double speed) {
    ...
    return speed
        * MathUtils.weightedAverage(
            RobotConstants.get().driveNormalMaxSpeed(),
            RobotConstants.get().driveFastMaxSpeed(),
            getLeftTriggerAxis());
  }

  /**
   * Returns the drive speed, with slow speed being taken into account.
   *
   * @param speed the drive speed without taking triggers into account.
   * @return the drive speed, with slow speed being taken into account.
   */
  public double turboSlowSpeed(final double speed) {
    ...
    return speed
        * MathUtils.weightedAverage(
            RobotConstants.get().driveNormalMaxSpeed(),
            RobotConstants.get().driveSlowMaxSpeed(),
            getRightTriggerAxis());
  }

  /**
   * Drive speed, using the left joystick's y-axis.
   *
   * @return the adjusted drive speed.
   * @see #getAdjustedSpeed(double)
   */
   ...
   *
   * @return the turn speed, which is slower when the robot is driving fast.
   * @see #getAdjustedSpeed(double)
   */
  public double getAdjustedTurnSpeed() {
    return getAdjustedSpeed(getRightX())
        * MathUtils.weightedAverage(
            RobotConstants.get().driveNormalTurnMaxSpeed(),
            RobotConstants.get().driveSlowTurnMaxSpeed(),
   ...
   *
   * @return the distance of this stick from the center position.
   */
  ...
  /** TODO: add javadoc comment for this method, although it is unused */
