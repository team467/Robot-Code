...
public class GoToTrajectoryCMD extends CommandBase {
  ...
  public GoToTrajectoryCMD(
    ...
    // Pass config
    this.trajectory =
        TrajectoryGenerator.generateTrajectory(initalPose, interiorWaypoints, endingPose, config);
  ...
  public void execute() {
    diffDriveOdometry.update(
        gyro.getRotation2d(), drivetrain.getLeftPosition(), drivetrain.getRightPosition());
    command.execute();
  }
