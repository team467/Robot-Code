...
public class PuppyModeCMD extends CommandBase {
  ...
  private final NetworkTableEntry hasBallEntry; // bool
  private final NetworkTableEntry distanceEntry; // double
  private final NetworkTableEntry angleEntry; // double
  ...
  public void execute() {
    if (hasBallEntry.getBoolean(false)) {
      ...
      double speed =
          MathUtil.clamp(distanceEntry.getDouble(0) / 120, 0.1, 0.5); // 120 inches max distance
