...
public class GoDistanceCMD extends CommandBase {
  ...
  public GoDistanceCMD(Drivetrain drivetrain, double distance) {
    ...
    this.leftPID =
        RobotConstants.get().driveDrivePositionPID().getProfiledPIDController(constraints);
    this.rightPID =
        RobotConstants.get().driveDrivePositionPID().getProfiledPIDController(constraints);
  ...
  public void execute() {
    System.out.printf(
        "Position: %f, Veclocity: %f%n",
        leftPID.getSetpoint().position, leftPID.getSetpoint().velocity);
    double leftAcceleration =
        (leftPID.getSetpoint().velocity - lastLeftSpeed) / (Timer.getFPGATimestamp() - lastTime);
    double rightAcceleration =
        (rightPID.getSetpoint().velocity - lastRightSpeed) / (Timer.getFPGATimestamp() - lastTime);

    double leftVoltage =
        MathUtil.clamp(leftPID.calculate(drivetrain.getLeftPosition(), distance), -1, 1)
            + driveFF.calculate(leftPID.getSetpoint().velocity, leftAcceleration);
    double rightVoltage =
        MathUtil.clamp(rightPID.calculate(drivetrain.getRightPosition(), distance), -1, 1)
