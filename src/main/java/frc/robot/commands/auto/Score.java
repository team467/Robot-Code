...
public class Score extends SequentialCommandGroup {
  ...
  public Score(String gamePieceType, String location, IntakeRelease intakeRelease) {
    add(gamePieceType, location, intakeRelease);
  }

  public Score(String location, IntakeRelease intakeRelease) {
    if (intakeRelease.getWants() == Wants.CONE) {
      add("Cone", location, intakeRelease);
    } else {
      add("Cube", location, intakeRelease);
    }
  }

  private void add(String gamePieceType, String location, IntakeRelease intakeRelease) {
    addCommands(
        pieceType(gamePieceType, intakeRelease),
        Commands.parallel(
            new IntakeCMD(intakeRelease)));
        // new ReleaseCMD(intakeRelease));
  }
