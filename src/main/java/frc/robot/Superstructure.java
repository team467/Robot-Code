...
public class Superstructure {
  ...
  public void superstructurePeriodic() {
    Logger.recordOutput("Superstructure/Pose", drive.getPose());
    Logger.recordOutput("Superstructure/CurrentZone", getCurrentZone().name());
    ...
    Logger.recordOutput(
        ...
        new Pose2d(
            ...
            Rotation2d.fromDegrees(0)));
    Logger.recordOutput("Superstructure/DistanceToHub", shootWhileDrivingResult.distance());
    Logger.recordOutput("Superstructure/ShooterPosition", shooterLeadCompensator.shooterPose());
    ...
    if (ballVision != null) {
      Logger.recordOutput("Superstructure/BallVision/HasTarget", ballVision.hasTarget());
      getClosestBallFieldPose()
          .get()
          .ifPresent(
              pose -> Logger.recordOutput("Superstructure/BallVision/ClosestBallFieldPose", pose));
    }
  }
  ...
  public Supplier<Optional<Pose2d>> getClosestBallFieldPose() {
    return () -> {
