...
/** TODO: replace this class with the WPILib new logger */
public class RobotLogManager {
    ...
    if (!initialized) {
      try {
        ...
        ConfigurationSource source =
            new ConfigurationSource(new FileInputStream(configSourceFile), configSourceFile);
        ...
        appenderBuilder.withFilePattern(
            pathToConfig.replace("log4j2.yaml", "logs") + "/app-%d{yyyy-MM-dd-HH-mm-ss-SSS}.log");
        ...
        org.apache.logging.log4j.core.appender.rolling.DefaultRolloverStrategy.Builder
            strategyBuilder = DefaultRolloverStrategy.newBuilder();
        strategyBuilder.withMax("50");
        appenderBuilder.withStrategy(strategyBuilder.build());
        org.apache.logging.log4j.core.layout.PatternLayout.Builder patternBuilder =
        ...
        configuration
            .getRootLogger()
            .addAppender(rollingRandomAccessFileAppender, Level.INFO, null);
        configuration
            .getLoggerConfig("frc.robot.Robot")
            .addAppender(rollingRandomAccessFileAppender, Level.INFO, null);
  ...
  /**
   * Initializes the log system if required, then returns the appropriate class logger. The custom
   * config file would normally be used for tests.
   *
