...
public class Drivetrain extends SubsystemTuner {
  ...
  public Drivetrain() {
    ...
    leftMotorLeader =
        MotorControllerFactory.create(
            RobotConstants.get().driveMotorLeftLeaderId(), RobotConstants.get().driveMotorType());
    rightMotorLeader =
        MotorControllerFactory.create(
            RobotConstants.get().driveMotorRightLeaderId(), RobotConstants.get().driveMotorType());
    ...
    leftMotorLeader.setUnitsPerRotation(
        Math.PI
            * RobotConstants.get().driveWheelDiameter()
            * RobotConstants.get().driveGearRatio().getRotationsPerInput());
    rightMotorLeader.setUnitsPerRotation(
        Math.PI
            * RobotConstants.get().driveWheelDiameter()
            * RobotConstants.get().driveGearRatio().getRotationsPerInput());
    ...
    if (RobotConstants.get().driveDualMotors()) {
      leftMotorFollower =
          MotorControllerFactory.create(
              RobotConstants.get().driveMotorLeftFollowerId(),
              RobotConstants.get().driveMotorType());
      rightMotorFollower =
          MotorControllerFactory.create(
              RobotConstants.get().driveMotorRightFollowerId(),
              RobotConstants.get().driveMotorType());
      ...
      leftMotorFollower.setUnitsPerRotation(
          Math.PI
              * RobotConstants.get().driveWheelDiameter()
              * RobotConstants.get().driveGearRatio().getRotationsPerInput());
      rightMotorFollower.setUnitsPerRotation(
          Math.PI
              * RobotConstants.get().driveWheelDiameter()
              * RobotConstants.get().driveGearRatio().getRotationsPerInput());
  ...
  /** Resets the estimator's position */
  ...
  public void enableGyro(Gyro gyro) {
    ...
    estimator.resetPosition(
        new Pose2d(getLeftPosition(), getRightPosition(), gyro.getRotation2d()),
        gyro.getRotation2d());
    field = new Field2d();
  ...
  public void periodic() {
    if (gyro != null) {
      estimator.update(
          gyro.getRotation2d(), getWheelSpeeds(), getLeftPosition(), getRightPosition());
      if (HubTarget.hasTarget()) {
        // TODO: vision measurement for hub
        estimator.addVisionMeasurement(
            new Pose2d(
                HubTarget.getRobotTranslation(), estimator.getEstimatedPosition().getRotation()),
            HubTarget.getTimestamp());
      }
  ...
  /**
   * Arcade drive method for differential drive platform, and supports velocity tuning
   *
    ...
    if (RobotConstants.get().driveUseVelocity()) {
      DifferentialDrive.WheelSpeeds speeds =
          DifferentialDrive.arcadeDriveIK(
              MathUtil.applyDeadband(speed, 0.02),
  ...
  /**
   * Curvature drive method for differential drive platform, and supports velocity tuning
   *
   ...
   * @param turnInPlace If set, overrides constant-curvature turning for turn-in-place maneuvers.
   *     zRotation will control turning rate instead of curvature.
   */
  ...
  /**
   * Gets the position of the left motor leader
   *
  ...
  /**
   * Gets the position of the right motor leader
   *
   * @return the position of the motor
   */
  ...
  /**
   * Gets the velocity of the left motor leader
   *
  ...
  /**
   * Gets the velocity of the right motor leader
   *
   * @return the velocity of the motor
   */
  ...
  /** Resets the left motor leader's position */
  private void resetLeftPosition() {
    leftMotorLeader.resetPosition();
  }

  /** Resets the right motor leader's position */
  private void resetRightPosition() {
    rightMotorLeader.resetPosition();
  }

  /** Resets the motor leaders' position */
  ...
  /** Resets the left drive PID */
  public void resetLeftPID() {
    leftDrivePID.reset();
  }

  /** Resets the right drive PID */
  ...
  public void initializeTuner() {
    ...
    new NetworkButton(getEntry("run"))
        .whileActiveContinuous(
            new ArcadeDriveCMD(
                this, () -> getEntry("speed").getDouble(0), () -> getEntry("turn").getDouble(0)));
  }
}
