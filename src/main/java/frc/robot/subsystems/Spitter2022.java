...
public class Spitter2022 extends SubsystemBase {
  ...
  public Spitter2022() {
    ...
    spitterMotor.setUnitsPerRotation(
        2.0 * Math.PI * RobotConstants.get().spitter2022GearRatio().getRotationsPerInput());
    ...
    //                    DCMotor.getNEO(1), RobotConstants.get().spitter2022MomentOfInertia(),
    // RobotConstants.get().spitter2022GearRatio().getRotationsPerOutput());
    spitterObserver =
        new KalmanFilter<>(
            Nat.N1(),
            Nat.N1(),
  ...
  public static double getFlywheelVelocity(double distance) {
    ...
    return RobotConstants.get().spitter2022MaxVelocity()
        * ((RobotConstants.get().spitter2022DistanceLinearM() * distance)
            + RobotConstants.get().spitter2022DistanceLinearB());
  }
  ...
  /** Sets the spitter velocity to the hub target. */
  public void setSpitterToTarget() {
    // System.out.printf("Setting velocity to %f rad/s, %f percent,l distance is %f meters, %n",
    // getFlywheelVelocity(), getFlywheelVelocity()/RobotConstants.get().spitter2022MaxVelocity(),
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty("Flywheel Velocity Error", () -> spitterLoop.getError(0), null);
  }
}
