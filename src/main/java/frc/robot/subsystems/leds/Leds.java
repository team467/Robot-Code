...
public class Leds extends SubsystemBase {
  ...
  public Leds() {
    ...
    // ledTestPatterns.setDefaultOption(LedPatterns.SOLID_BLACK.toString(),
    // LedPatterns.SOLID_BLACK);
    ledTestPatterns.setDefaultOption(LedPatterns.RAINBOW.toString(), LedPatterns.RAINBOW);
    ...
    loadingNotifier =
        new Notifier(
            () -> {
              LedPatterns.SOLID_RED.colorPatternOnly().applyTo(buffer);
              leds.setData(buffer);
            });
  ...
  public void periodic() {
    ...
    LedPatterns pattern = LedPatterns.SOLID_COM_GOLD;
    ...
    if (ledTestingEntry.getBoolean(false)) {
      try {
        ...
        pattern = ledTestPatterns.getSelected();
        mode = LedMode.valueOf(ledModeEntry.getString("OFF"));
      } catch (IllegalArgumentException E) {
      }
    } else {
      pattern = LedPatterns.SOLID_COM_GOLD; // TODO: connect to state
    }
    ...
    pattern.colorPatternOnly().applyTo(buffer);
    leds.setData(buffer);
  }
}
