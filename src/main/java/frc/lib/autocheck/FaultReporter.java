...
/**
 * The FaultReporter class is responsible for detecting and reporting faults in the robot subsystems.
 * It provides methods for registering system checks, adding faults, clearing faults, and checking for faults.
 * The class maintains a list of subsystem faults and a list of self-checking hardware components.
 * The system status and faults are periodically published to the logger for monitoring.
 */
public class FaultReporter {
    ...
    /**
     * Publishes the status of each subsystem by recording output in the logger.
     * It records the status, system OK indicator, faults, and last fault for each subsystem.
     */
     ...
     */
    public void registerHardware(
            String subsystemName, String label, Pigeon2 pigeon2) {
        SubsystemFaults subsystemFaults =
                subsystemsFaults.getOrDefault(subsystemName, new SubsystemFaults());
     ...
     * <p>If checkErrors is set to true, it iterates through each subsystem and its registered hardware components,
     * and calls the checkForFaults() method on each registered hardware component.
     * <p>Any detected faults are added to the list of faults for the respective subsystem.
     *
     * <p>Note: The checkErrors flag must be set to true before calling this method, otherwise no faults will be checked.
     */
